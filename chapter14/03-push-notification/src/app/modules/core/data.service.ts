...
export class DataService {
  ...
  // this is the original endpoint https://us-central1-awesome-apress-pwa.cloudfunctions.net/saveNote
  // however for the sake of Background Sync test, I have set up simple Express server
  // which proxy this endpoint and run the while application.
  // This Route will not do anything in production or if you deploy to Firebase,
  // consider to replace this endpoint with the original one if you want to deploy to firebase
  ...
  constructor(private afDb: AngularFirestore, private http: HttpClient, private auth: AuthService) {}
  ...
  getUserNotesCollection() {
    return this.afDb.collection(this.USERS_COLLECTION + '/' + this.auth.id + '/' + this.NOTES_COLLECTION, ref =>
  ...
  getNoteFromDirectApi(id): Observable<any> {
    return this.auth.getToken().pipe(
      switchMap(idToken => {
        return this.http.get(`${this.FIRESTORE_ENDPOINT}users/${this.auth.id}/notes/${id}`, {
          headers: {
            Authorization: `Bearer ${idToken}`
          }
        });
  ...
  initializeNotes(): Observable<any> {
    return this.auth.getToken().pipe(
      switchMap(idToken => {
        return this.http.get(`${this.FIRESTORE_ENDPOINT}users/${this.auth.id}/notes`, {
          headers: {
            Authorization: `Bearer ${idToken}`
          }
        });
  ...
  private transfromNote(note) {
    ...
    for (const prop in note.fields) {
      if (note.fields[prop]) {
        _note[prop] = note.fields[prop]['stringValue'] || note.fields[prop]['integerValue'];
      }
    }
    return _note;
  }
